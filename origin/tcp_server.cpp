...
void server::HandleClient(int conn)
{
    ...
    while (1) {
        ...
        if (str=="work")
        {
            ...
            std::thread t1(server::Handler,"Cpp is",1,std::ref(current_thread));
            std::thread t2(server::Handler,"the best",2,std::ref(current_thread));
            std::thread t3(server::Handler,"language",3,std::ref(current_thread));
            t1.join();
            t2.join();
            t3.join();
        }
...
void server::Handler(std::string str,int number,int &current_thread)
{
    std::unique_lock<std::mutex> lock(mtx);
    // std::cout<<current_thread<<std::endl;

    cv.wait(lock,[current_thread,number]{return number==current_thread;});
